...
create table if not exists public.atendimentos (
  ...
  created_by uuid not null references public.profiles(id),
  setor text not null check (setor in ('Documental', 'Topografia', 'Projetos', 'Financeiro', 'Geral')),
...
declare p record;
begin
  for p in select schemaname, tablename, policyname from pg_policies where schemaname = 'public' and tablename in ('profiles','clientes','atendimentos','tarefas','historico','marketing_etapas','marketing_projetos','marketing_progresso')
  loop
