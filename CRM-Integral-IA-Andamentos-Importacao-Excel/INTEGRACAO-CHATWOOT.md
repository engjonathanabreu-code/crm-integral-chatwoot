...
## Consulta de Andamentos pelo Agente IA

O CRM agora expõe `POST /api/andamento-cliente` para consulta somente-leitura pelo agente.
Envie `Authorization: Bearer <CRM_AGENT_READ_SECRET>` e um dos identificadores: `phone`, `chatwoot_contact_id` ou `conversation_id`.

A resposta contém apenas andamentos com `visivel_ia=true`. O campo `observacao_interna` nunca é retornado. A IA deve usar `descricao_cliente`, `etapa`, `status_operacional`, `orientacao_ia`, `previsao` e `atualizado_em` como fonte factual. Se não houver previsão, a IA não deve estimar prazo.
